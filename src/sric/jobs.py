...
from .errors import safe_exception_message, safe_operational_text
...
def _safe_mapping(value: dict[str, Any] | None) -> dict[str, Any]:
    safe, _ = redact_structure(value or {})
    if not isinstance(safe, dict):
        raise TypeError("redacted job metadata must remain a mapping")
    return safe
...
class JobEngine:
    ...
    def create(
        ...
        *,
        ...
        existing = {job.job_id for job in self.list()}
        missing = [dependency for dependency in deps if dependency not in existing]
        if missing:
            raise ValueError(f"unknown job dependencies: {missing}")
        job = Job(
            job_type=job_type,
    ...
    def dependencies_satisfied(self, job_id: str) -> bool:
        return all(
            self.get(dependency).status == JobStatus.COMPLETED
            for dependency in self.get(job_id).dependencies
        )
    ...
    def ready(self) -> list[Job]:
        return [
            ...
            if job.status == JobStatus.QUEUED and self.dependencies_satisfied(job.job_id)
        ]
    ...
    def transition(
        ...
        *,
        ...
        for index, raw in enumerate(data["jobs"]):
            ...
            if status == JobStatus.RUNNING and not self.dependencies_satisfied(job_id):
                raise ValueError("job dependencies are not completed")
            ...
            if status == JobStatus.RUNNING:
                job.attempt += 1
            job.error = safe_operational_text(error) if error is not None else None
            job.updated_at = utcnow()
            data["jobs"][index] = job.model_dump(mode="json")
            self._save(data)
            self.event(
                job_id,
                "status",
                f"Job transitioned to {status}",
    ...
    def retry(self, job_id: str) -> Job:
        ...
        if job.attempt >= job.budget.max_attempts:
            raise PermissionError("job retry budget exhausted")
    ...
    def add_artifact(self, job_id: str, artifact_id: str) -> Job:
        ...
        for index, raw in enumerate(data["jobs"]):
            if raw["job_id"] != job_id:
                continue
            job = Job.model_validate(raw)
            if len(job.artifacts) >= job.budget.max_artifacts:
                raise PermissionError("job artifact budget exceeded")
            if artifact_id not in job.artifacts:
                job.artifacts.append(artifact_id)
            job.updated_at = utcnow()
            data["jobs"][index] = job.model_dump(mode="json")
            self._save(data)
            self.event(job_id, "artifact", "Artifact recorded", {"artifact_id": artifact_id})
            return job
        raise KeyError(job_id)
    ...
    def event(
        ...
        event = JobEvent(
            ...
            message=safe_operational_text(message),
            data=_safe_mapping(data),
        )
        store = self._load()
        store["events"].append(event.model_dump(mode="json"))
        self._save(store)
        return event

    def events(self, job_id: str) -> list[JobEvent]:
        return [
            JobEvent.model_validate(item)
            for item in self._load()["events"]
    ...
    def all_events(self, after: int = 0) -> list[JobEvent]:
        ...
        return [JobEvent.model_validate(item) for item in self._load()["events"][after:]]

    def list(self) -> list[Job]:
        return [Job.model_validate(item) for item in self._load()["jobs"]]

    def dag(self) -> dict[str, Any]:
        jobs = self.list()
        return {
            "nodes": [job.model_dump(mode="json") for job in jobs],
            "edges": [
                {"from": dependency, "to": job.job_id}
                for job in jobs
...
class JobContext:
    ...
    def cancelled(self) -> bool:
        return self.engine.get(self.job_id).status in {
            JobStatus.CANCELLING,
            JobStatus.CANCELLED,
        }
    ...
    def artifact(self, artifact_id: str) -> Job:
        return self.engine.add_artifact(self.job_id, artifact_id)
...
class AsyncJobRunner:
        ...
        *,
        ...
        async def run() -> None:
            async with self._semaphore:
                if self.engine.get(job.job_id).status == JobStatus.CANCELLED:
                    return
                if not self.engine.dependencies_satisfied(job.job_id):
                    return
                self.engine.transition(job.job_id, JobStatus.RUNNING)
                context = JobContext(self.engine, job.job_id)
                try:
    ...
    def recover_orphans(self) -> list[str]:
        ...
        for job in self.engine.list():
            if (
                job.status in {JobStatus.RUNNING, JobStatus.PAUSED, JobStatus.CANCELLING}
                and job.job_id not in self._tasks
            ):
                reason = (
                    "worker ended; resumable job requires explicit retry"
                    if job.resumable
                    else "worker process ended before job completion"
                )
                self.engine.transition(job.job_id, JobStatus.FAILED, error=reason)
