...
def _validate_references(nodes: Sequence[ExportNode], edges: Sequence[ExportEdge]) -> None:
    node_ids = [node.node_id for node in nodes]
    edge_ids = [edge.edge_id for edge in edges]
    if len(node_ids) != len(set(node_ids)):
        raise ValueError("exported node IDs must be unique")
    if len(edge_ids) != len(set(edge_ids)):
        raise ValueError("exported edge IDs must be unique")
    known = set(node_ids)
    for edge in edges:
...
def export_jsonld(nodes: Sequence[ExportNode], edges: Sequence[ExportEdge]) -> str:
    _validate_references(nodes, edges)
...
def export_graphml(nodes: Sequence[ExportNode], edges: Sequence[ExportEdge]) -> str:
    _validate_references(nodes, edges)
    ...
    for node in sorted(nodes, key=lambda item: item.node_id):
        ...
        SubElement(element, "data", key="node_evidence").text = json.dumps(sorted(node.evidence_ids), separators=(",", ":"))
        SubElement(element, "data", key="node_attributes").text = json.dumps(node.attributes, sort_keys=True, separators=(",", ":"), ensure_ascii=False, default=str)
    for edge in sorted(edges, key=lambda item: item.edge_id):
        element = SubElement(graph, "edge", id=edge.edge_id, source=edge.source_id, target=edge.target_id)
