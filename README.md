...
Installer UX is intentionally compact: the user-facing `doctor` validation may render the SRIC banner once, while internal `capabilities`, `--help`, `-h`, and `help` smoke checks run with `SENTINEL_BANNER=off`. Normal interactive CLI commands are unchanged and continue to show the banner according to the standard presentation policy.
...
The 0.5.11 installer regressions verify single-banner installation output in addition to venv-scoped runtime recreation, Termux `$PREFIX/bin` selection, safe Windows registry PATH handling, Windows Python selection, dependency integrity checks, shared Web import probes and help forms. The existing exhaustive interface gate continues to walk every public command and verify CLI/Web parameter parity.
