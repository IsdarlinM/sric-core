...
def test_unresolved_upstream_is_reported() -> None:
    report = resolve_source_independence([profile("derived", upstream=["missing-provider"])])
...
def test_dependency_cycle_is_conservatively_reported() -> None:
    ...
    assert report.source_groups["one"] == report.source_groups["two"]
    assert report.independent_group_count == 1
    assert any("cycles" in item for item in report.limitations)


def test_cycle_dependency_overrides_declared_independence_group() -> None:
    report = resolve_source_independence([
        profile("one", upstream=["two"], group="manual"),
        profile("two", upstream=["one"]),
    ])
    assert report.source_groups["one"] == report.source_groups["two"]
    assert report.independent_group_count == 1
